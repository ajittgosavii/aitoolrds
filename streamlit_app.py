...
with col1:
    ...
    # Also show AWS credentials status if Boto3 is available
    if HAS_BOTO3:
...
with col2:
    ...
    if HAS_BOTO3 and st.button("🔄 Refresh AWS Status"):
...
if 'results' in st.session_state:
    ...
    if valid_results:
        ...
        if HAS_PLOTLY:
            ...
            st.subheader("vCPU Allocation")
            st.bar_chart(resource_df.set_index('Environment')[['Writer vCPUs', 'Reader vCPUs (Total)']])
            
            st.subheader("RAM Allocation")
            st.bar_chart(resource_df.set_index('Environment')[['Writer RAM', 'Reader RAM (Total)']])
...
- ✅ **Dependency Handling**: Graceful degradation when optional packages are missing
""")